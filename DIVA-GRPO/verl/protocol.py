...
def union_tensor_dict(tensor_dict1: TensorDict, tensor_dict2: TensorDict) -> TensorDict:
    """Union two tensordicts, truncate the larger batch to match the smaller one."""
    bs1, bs2 = tensor_dict1.batch_size[0], tensor_dict2.batch_size[0]

    if bs1 != bs2:
        min_bs = min(bs1, bs2)
        if bs1 > min_bs:
            tensor_dict1 = tensor_dict1[:min_bs]
        if bs2 > min_bs:
            tensor_dict2 = tensor_dict2[:min_bs]

    for key in tensor_dict2.keys():
        if key in tensor_dict1 and not torch.equal(tensor_dict1[key], tensor_dict2[key]):
            raise ValueError(f"Key already exists with different value: {key}.")
...
class DataProto:
    ...
    # 创建带有torch.Tensor、non_tensors、meta_info的DataProto
    @classmethod
    ...
    def chunk(self, chunks: int) -> List["DataProto"]:
        """Split the batch among dim=0 into equal chunks (drop remainder).
        ...
        """
        size = len(self)
        new_size = size - (size % chunks)  # 丢掉多余部分
        if new_size == 0:
            raise ValueError(f"DataProto too small: size={size}, chunks={chunks}")

        if self.batch is not None:
            batch_trimmed = self.batch[:new_size]
            batch_lst = batch_trimmed.chunk(chunks=chunks, dim=0)
        else:
            batch_lst = [None for _ in range(chunks)]

        non_tensor_batch_lst = [{} for _ in range(chunks)]
        for key, value in self.non_tensor_batch.items():
            value_trimmed = value[:new_size]  # 同样丢掉多余的
            non_tensor_lst = np.array_split(value_trimmed, chunks)
